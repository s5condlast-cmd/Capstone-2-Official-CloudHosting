...
describe('Plate editor runtime wiring', () => {
  ...
  it('enforces full-margin header/footer box containers, default fullscreen, and elevated z-index popovers/menus', async () => {
    ...
    // 1. Full-margin box containers for Header & Footer with dynamic expansion up to 288px / 180px and 48px top spacing
    ...
    assert.ok(headerSrc.includes('HEADER_TOP_SPACING = 48'), 'Header top spacing must be configured to 48px (0.5 inch)');
    assert.ok(headerSrc.includes('HEADER_MAX_HEIGHT = 288'), 'Header max height must be configured to 288px');
    assert.ok(headerSrc.includes('pt-[48px]'), 'Header container must include 48px top breathing room padding');
