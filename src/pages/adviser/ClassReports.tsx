...
export const ClassReports: React.FC = () => {
  const [activeTab, setActiveTab] = useState<'progress' | 'companies'>('progress');
  ...
  const companyStats = [
    { name: 'InnoTech Labs', students: 18, completed: 17, dropped: 1, rating: 4.7, hours: 462, topSkill: 'Communication' },
    { name: 'TechCorp Solutions Inc.', students: 12, completed: 10, dropped: 0, rating: 4.5, hours: 460, topSkill: 'Technical Execution' },
    { name: 'Pixel Perfect Co.', students: 6, completed: 4, dropped: 2, rating: 4.2, hours: 458, topSkill: 'Design Thinking' },
    { name: 'CloudNet Systems', students: 4, completed: 4, dropped: 0, rating: 4.8, hours: 465, topSkill: 'Problem Solving' }
  ];
    ...
    <div className="space-y-8 pb-12">
      ...
      <div className="flex border-b border-zinc-200 dark:border-zinc-800 gap-4 mb-6">
        <button
          ...
          className={cn(
            "pb-3 text-xs font-bold uppercase tracking-wider transition-all border-b-2 px-1",
            activeTab === 'progress'
      ...
      {activeTab === 'progress' ? (
        <>
                      ...
                      >
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-3 min-w-[100px]">
                            ...
                            <span className="text-xs font-bold text-zinc-900 dark:text-zinc-100 tabular-nums w-8 text-right">{progressPct}%</span>
                          </div>
                        </td>
                        <td className="px-6 py-4">
                          <Badge variant={s.status === 'on-track' ? 'success' : s.status === 'behind' ? 'warning' : 'error'}>
                            {s.status === 'on-track' ? 'On Track' : s.status === 'behind' ? 'Behind' : 'At Risk'}
                          </Badge>
                        </td>
                      </motion.tr>
                    );
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {/* Insights Card */}
            <Card title="Cohort Analytics" className="md:col-span-2">
              <div className="flex flex-col sm:flex-row gap-5 items-start">
                <div className="w-12 h-12 rounded-xl bg-zinc-100 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 flex items-center justify-center text-zinc-500 shrink-0">
                  <Sparkles size={22} className="text-zinc-900 dark:text-white" />
                </div>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <StatCard label="Partner Companies" value={companyStats.length.toString()} icon={<Building size={18} />} />
            <StatCard label="Avg. Partner Rating" value="4.6 / 5.0" icon={<Star size={18} />} />
            <StatCard label="Total Deployed Students" value="40" icon={<Users size={18} />} />
          </div>

          <Card title="Industry Partner Metrics" className="overflow-hidden">
            <div className="overflow-x-auto -mx-6">
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-zinc-50 dark:bg-zinc-900 border-y border-zinc-100 dark:border-zinc-800/50">
                    ...
                    <th className="px-6 py-3 text-[10px] font-bold text-zinc-400 dark:text-zinc-500 uppercase tracking-wide text-center">Completed</th>
                    <th className="px-6 py-3 text-[10px] font-bold text-zinc-400 dark:text-zinc-500 uppercase tracking-wide text-center">Dropped</th>
                    <th className="px-6 py-3 text-[10px] font-bold text-zinc-400 dark:text-zinc-500 uppercase tracking-wide text-center">Avg Appraisal Score</th>
                    <th className="px-6 py-3 text-[10px] font-bold text-zinc-400 dark:text-zinc-500 uppercase tracking-wide text-center">Avg Rendered Hours</th>
                    <th className="px-6 py-3 text-[10px] font-bold text-zinc-400 dark:text-zinc-500 uppercase tracking-wide">Top Identified Skill</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-zinc-50 dark:divide-zinc-800/30">
                  {companyStats.map((co, i) => (
                    <tr key={i} className="hover:bg-zinc-50/50 dark:hover:bg-zinc-800/30 transition-colors">
                      <td className="px-6 py-4 font-bold text-sm text-zinc-900 dark:text-zinc-100">{co.name}</td>
                      <td className="px-6 py-4 text-center text-xs font-bold text-zinc-700 dark:text-zinc-300">{co.students}</td>
                      <td className="px-6 py-4 text-center text-xs font-semibold text-green-600 dark:text-green-400">{co.completed}</td>
                      <td className="px-6 py-4 text-center text-xs font-semibold text-red-600 dark:text-red-400">{co.dropped}</td>
                      <td className="px-6 py-4 text-center text-xs font-bold text-zinc-800 dark:text-zinc-200">
                        <div className="flex items-center justify-center gap-1">
                          <Star size={12} className="fill-current text-yellow-500" />
                          <span>{co.rating.toFixed(1)} / 5.0</span>
                        </div>
                      </td>
                      <td className="px-6 py-4 text-center text-xs font-bold text-zinc-800 dark:text-zinc-200">{co.hours} hrs</td>
                      <td className="px-6 py-4 text-xs font-semibold text-zinc-600 dark:text-zinc-400">{co.topSkill}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
