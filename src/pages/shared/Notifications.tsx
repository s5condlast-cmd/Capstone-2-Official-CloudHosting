  ...
  return (
    <div className="space-y-8 pb-12">
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        {/* Left Column: Notifications Feed (12 cols by default, 8 cols when settings open) */}
        <div className={cn(isSettingsOpen ? "lg:col-span-8" : "lg:col-span-12", "transition-all duration-300")}>
          <div className="bg-white dark:bg-zinc-950 border border-zinc-200/80 dark:border-zinc-800/80 rounded-2xl p-6 shadow-xs space-y-5">
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2 text-zinc-400 dark:text-zinc-500">
                <button title="Mark all as read" className="p-1.5 hover:text-zinc-900 dark:hover:text-white hover:bg-zinc-100 dark:hover:bg-zinc-900 rounded-lg transition-colors cursor-pointer">
                  <CheckCheck size={18} />
                </button>
                <button 
                  onClick={() => setIsSettingsOpen(prev => !prev)}
                  title={isSettingsOpen ? "Close settings (Active)" : "Notification settings (Hidden)"} 
                  className={cn(
            ...
            <div className="space-y-6 pt-2">
              {(activeTab === 'revisions' || activeTab === 'all') && (
                ...
                >
                  <div className="flex items-center gap-3 text-xs">
                    <div className="w-8 h-8 rounded-full border border-zinc-200 dark:border-zinc-800 bg-zinc-100 dark:bg-zinc-900 flex items-center justify-center text-zinc-900 dark:text-zinc-100 font-bold shrink-0 shadow-2xs group-hover:scale-105 transition-transform">
                      <UserIcon size={15} />
                    </div>
              ...
              {(activeTab === 'verified' || activeTab === 'all') && (
                <div 
                  onClick={() => { if (isSettingsOpen) setIsSettingsOpen(false); }}
                  className="space-y-2.5 pt-4 border-t border-zinc-100 dark:border-zinc-800/60 cursor-pointer group"
                >
                  <div className="flex items-center gap-3 text-xs">
                    <div className="w-8 h-8 rounded-full border border-zinc-200 dark:border-zinc-800 bg-zinc-100 dark:bg-zinc-900 flex items-center justify-center text-zinc-900 dark:text-zinc-100 font-bold shrink-0 shadow-2xs group-hover:scale-105 transition-transform">
        ...
        {/* Right Column: Notification Settings & Policy (Triggered by Settings Gear Button) */}
        {isSettingsOpen && (
          <div className="lg:col-span-4 space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
            <Card 
