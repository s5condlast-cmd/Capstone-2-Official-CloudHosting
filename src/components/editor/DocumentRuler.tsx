...
export const DocumentRuler: React.FC<DocumentRulerProps> = ({
  ...
  return (
    ...
    >
      ...
      <div className="relative w-full h-full flex bg-card border-x border-t border-border rounded-t-xs overflow-hidden shadow-2xs">
        {/* Left Margin Gutter (Shaded 1 inch / 96px) */}
        <div
          style={{ width: `${leftMargin}px` }}
          className="h-full bg-muted/70 border-r border-border/80 relative shrink-0"
        />

        {/* Central Printable Area */}
        <div
          style={{ width: `${printableWidth}px` }}
          className="h-full bg-card relative shrink-0"
        />

        {/* Right Margin Gutter (Shaded 1 inch / 96px) */}
        <div
          style={{ width: `${rightMargin}px` }}
          className="h-full bg-muted/70 border-l border-border/80 relative shrink-0"
        />
              ...
              <React.Fragment key={index}>
                ...
                <div
                  className={cn(
                    ...
                    isInch && 'h-2.5 bg-foreground',
                    isHalfInch && 'h-2 bg-muted-foreground',
                    isQuarterInch && 'h-1.5 bg-muted-foreground/70',
                    !isInch && !isHalfInch && !isQuarterInch && 'h-1 bg-border'
                  )}
                  style={{ left: `${px}px` }}
                />
                ...
                {showNumber && (
                  <span
                    className="absolute top-0.5 text-[9px] font-mono font-medium text-muted-foreground -translate-x-1/2 select-none"
                    style={{ left: `${px}px` }}
                  >
                    {inchNumber}
                  </span>
